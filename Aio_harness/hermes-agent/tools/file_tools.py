...
def list_dir_tool(path: str = ".", task_id: str = "default") -> str:
    """List the immediate entries of a directory under the task's workspace.

    Not exposed to the agent's tool registry — used only by the gateway's
    read-only ``/v1/workspace/tree`` HTTP route (FE file browser), reusing
    the same path resolution and sensitive-path guard as ``read_file_tool``.
    """
    try:
        if _is_blocked_device(path):
            return json.dumps({"error": f"Cannot list '{path}': blocked device path."})

        resolved = _resolve_path_for_task(path, task_id)

        sensitive = _check_sensitive_path(str(resolved), task_id)
        if sensitive:
            return json.dumps({"error": sensitive})
