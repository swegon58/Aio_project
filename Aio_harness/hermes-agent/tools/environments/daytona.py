...
class DaytonaEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        if self._sandbox is None:
            ...
            if network_allow_list:
                create_kwargs["network_allow_list"] = network_allow_list
            self._sandbox = self._daytona.create(
                CreateSandboxFromImageParams(**create_kwargs)
            )
    ...
    def download_file(self, remote_path: str, local_path: str) -> bool:
        """Fetch one arbitrary file from the sandbox (e.g. a code-execution artifact).

        ``sandbox.fs.*`` calls go through Daytona's separate toolbox HTTP
        daemon, which runs as a fixed, less-privileged user — it can't
        traverse root-owned directories like ``/root`` (mode 700) even
        though ``process.exec()`` (which runs as root) can read the same
        file fine. Fall back to an exec-based read for that case instead of
        depending on the toolbox daemon's permissions.
        """
        try:
            Path(local_path).parent.mkdir(parents=True, exist_ok=True)
            self._sandbox.fs.download_file(remote_path, local_path)
            return os.path.isfile(local_path)
        except Exception:
            logger.warning(
                "Daytona fs.download_file failed for %s, falling back to exec-based read",
                remote_path,
                exc_info=True,
            )
            return self._download_file_via_exec(remote_path, local_path)

    def _download_file_via_exec(self, remote_path: str, local_path: str) -> bool:
        """Read a remote file's bytes via process.exec (base64) instead of the
        toolbox fs API, sidestepping the toolbox daemon's separate permission
        context entirely."""
        try:
            result = self.execute(
            ...
            logger.warning("Daytona exec-based download fallback failed for %s", remote_path, exc_info=True)
            return False
