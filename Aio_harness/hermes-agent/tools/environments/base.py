...
class BaseEnvironment(ABC):
    ...
    def download_file(self, remote_path: str, local_path: str) -> bool:
        """Fetch a single file from the backend's filesystem to *local_path*.

        Only meaningful for remote backends (Daytona, Modal, SSH) whose
        filesystem the gateway host can't see directly. Bind-mount backends
        (Docker, Singularity, Local) already share a filesystem view with the
        host, so the default no-op is correct for them.
        """
        return False
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        """Wrap command in a brace group with stdin_data as a heredoc.

        ``command`` is often a ``;``-joined sequence of statements (e.g.
        write_file's atomic-write script). A heredoc appended directly after
        such a string binds only to the *last* simple command in it, not
        whichever command inside actually reads stdin (e.g. a `cat` in the
        middle) — bash redirections on compound lists attach to the trailing
        command only. Wrapping in `{ ...; }` makes the whole group share one
        stdin, so any command in it that reads stdin without its own
        redirection gets the heredoc content as intended.
        """
        delimiter = f"HERMES_STDIN_{uuid.uuid4().hex[:12]}"
        body = stdin_data if stdin_data.endswith("\n") else stdin_data + "\n"
        return f"{{\n{command}\n}} << '{delimiter}'\n{body}{delimiter}"
