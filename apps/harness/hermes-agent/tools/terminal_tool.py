...
def get_active_environment(task_id: Optional[str]):
    """Look up the already-running environment for *task_id*, if any.

    Read-only counterpart to ``_get_or_create_env`` — never creates a new
    sandbox. Used by the gateway to fetch artifacts a tool wrote inside a
    remote sandbox's filesystem (e.g. Daytona) after the run that created
    them has already finished.
    """
    effective_task_id = _resolve_container_task_id(task_id)
    with _env_lock:
        return _active_environments.get(effective_task_id)
