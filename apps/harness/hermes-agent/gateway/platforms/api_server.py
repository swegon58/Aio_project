...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # File paths surfaced by tool.completed events during a run (e.g.
        # generated .pptx/.xlsx from skill scripts), allow-listed per run_id
        # so /v1/runs/{run_id}/file can stream them without becoming an
    ...
    def _create_agent(
        ...
        if enabled_toolsets_override is not None:
            # Caller wants a hard cap on tool access for this run (e.g. plan
            # mode), independent of model obedience to the system prompt.
    ...
    # Office/export extensions surfaced as chat attachments — deliberately
    # narrow (not arbitrary file types) so this can't be tricked into
    # surfacing source code, secrets, or config as "downloadable" output.
    _ARTIFACT_EXTENSIONS = (".pptx", ".xlsx", ".docx", ".csv", ".pdf")

    @classmethod
    def _extract_artifact_path(cls, result: Any) -> Optional[str]:
        """Pull a generated-file path out of a tool result, if present.

        Handles both ``write_file_tool``'s JSON (``resolved_path``/
        ``files_modified``) and plain-text terminal output that mentions an
        absolute path (e.g. a skill script printing where it saved a
        ``.pptx``). Returns ``None`` when nothing matches.
        """
        if not result:
            return None
        ...
        try:
            ...
            for key in ("resolved_path", "files_modified"):
                val = data.get(key) if isinstance(data, dict) else None
                if isinstance(val, list) and val:
                    val = val[0]
                if isinstance(val, str) and val.lower().endswith(cls._ARTIFACT_EXTENSIONS):
                    return val
        except (json.JSONDecodeError, TypeError):
            pass
        for match in re.finditer(r"(/[^\s\"'<>]+(?:" + "|".join(re.escape(e) for e in cls._ARTIFACT_EXTENSIONS) + r"))", text):
    ...
    def _make_run_event_callback(self, run_id: str, loop: "asyncio.AbstractEventLoop"):
        ...
        def _callback(event_type: str, tool_name: str = None, preview: str = None, args=None, **kwargs):
            ...
            if event_type == "tool.started":
                ...
                artifact_path = self._extract_artifact_path(kwargs.get("result"))
                if artifact_path and not kwargs.get("is_error"):
                    self._run_artifacts.setdefault(run_id, set()).add(artifact_path)
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        instructions = body.get("instructions")
        previous_response_id = body.get("previous_response_id")
        # Plan mode / "questions only" runs: caller can request a hard,
        # non-bypassable tool lockout instead of relying on the model to
        # honor "do not call tools" in `instructions` — small local models
        # don't reliably follow that and will execute anyway.
        disable_tools = bool(body.get("disable_tools"))
        ...
        if not conversation_history and isinstance(raw_input, list) and len(raw_input) > 1:
            for idx, msg in enumerate(raw_input[:-1]):
                if isinstance(msg, dict) and msg.get("role") and msg.get("content"):
                    try:
                        content = _normalize_multimodal_content(msg["content"])
                    except ValueError as exc:
                        return _multimodal_validation_error(exc, param=f"input[{idx}].content")
    ...
    async def _handle_workspace_tree(self, request: "web.Request") -> "web.Response":
        """GET /v1/workspace/tree?task_id=&path= — list a workspace directory.

        Read-only. Reuses ``tools.file_tools.list_dir_tool``'s path
        resolution and sensitive-path guard — same scoping as the agent's own
        file tools, not a new trust boundary.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        from tools.file_tools import list_dir_tool

        result = await asyncio.get_running_loop().run_in_executor(
            None, list_dir_tool, path, task_id
        )
    ...
    async def _handle_run_file(self, request: "web.Request") -> "web.Response":
        ...
        Not an arbitrary-path-read endpoint: ``path`` must exactly match one
        the run itself surfaced via a ``tool.completed`` event (see
        ``_extract_artifact_path``), recorded in ``self._run_artifacts``.
        Used by Aio's chat-bubble attachment card to deliver files skill
        scripts wrote (e.g. a generated .pptx/.xlsx) without a general
        ...
        run_id = request.match_info["run_id"]
        path = request.query.get("path") or ""
        allowed = self._run_artifacts.get(run_id, set())
        if not path or path not in allowed:
            return web.json_response(
                _openai_error(f"File not found for run: {run_id}", code="artifact_not_found"),
                status=404,
            )
        local_path = path
        if not os.path.isfile(path):
            local_path = self._try_fetch_remote_artifact(run_id, path)
            if not local_path:
                return web.json_response(
                    _openai_error("Artifact no longer exists on disk", code="artifact_missing"),
                    status=404,
                )
        return web.FileResponse(local_path)

    def _try_fetch_remote_artifact(self, run_id: str, path: str) -> Optional[str]:
        """Best-effort on-demand download for artifacts living in a remote
        sandbox (e.g. Daytona) that never synced back to the gateway host.

        Looks up the live environment for the run's task (same id derived at
        run-start: ``session_id or run_id``) and, if it supports
        ``download_file`` (only remote backends do), pulls *path* down into a
        per-run cache dir under the system temp dir — NOT to ``path`` itself.
        The remote path is an absolute path inside the sandbox (e.g.
        ``/root/test.csv``); writing to that same absolute path on the
        gateway host would collide with real, often permission-restricted,
        host directories (the host's own ``/root`` isn't writable by the
        gateway's non-root user). Returns the local cache path on success,
        or None (caller falls back to the existing 404).
        """
        try:
            from tools.terminal_tool import get_active_environment

            status = self._run_statuses.get(run_id) or {}
            effective_task_id = status.get("session_id") or run_id
            env = get_active_environment(effective_task_id)
            if env is None:
                return None
            cache_dir = Path(tempfile.gettempdir()) / "hermes_artifacts" / run_id
            cache_dir.mkdir(parents=True, exist_ok=True)
            local_path = str(cache_dir / os.path.basename(path))
            if env.download_file(path, local_path):
                return local_path
            return None
        except Exception:
            logger.warning("On-demand artifact fetch failed for run %s", run_id, exc_info=True)
            return None
