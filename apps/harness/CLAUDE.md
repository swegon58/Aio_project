...
- Runs as Hermes **profile "aio"** (`apps/harness/aio-home/profiles/aio`, launched with `HERMES_HOME=aio-home -p aio`) — fully isolated instance: own config, API keys, memory, sessions, skills.
...
- Aio (`../web`, Next.js) is the UI/product. This harness is the brain it calls into — keep them decoupled (API/MCP boundary, not direct imports).
