...
//
// TODO (Q41 — Vault wiring, separate task): real per-customer secrets
// (OpenRouter key with hard spend ceiling, Honcho config) must come from
// Supabase Vault. Until that wiring exists, this sources:
//   - OPENROUTER_API_KEY + DAYTONA_API_KEY from the Aio server's own
//     "aio" dev profile .env (shared dev key, no per-customer ceiling yet)
//   - API_SERVER_KEY: generated fresh per profile (real, not a placeholder)
//   - TERMINAL_ENV=daytona (matches Q7)
async function writeProfileEnv(profileName: string): Promise<string> {
  const devEnvPath = profileEnvPath(DEV_PROFILE_NAME);
  const devEnvRaw = await fs.readFile(devEnvPath, "utf-8");

  const openrouterKey = devEnvRaw.match(/^OPENROUTER_API_KEY=(.*)$/m)?.[1] ?? "";
  ...
  const lines = [
    ...
    "# TODO (Q41): replace OPENROUTER_API_KEY/DAYTONA_API_KEY with per-customer",
    "# Supabase-Vault-sourced values + hard spend ceiling. Currently sharing the",
    "# Aio dev profile's keys as a Phase-1 placeholder.",
    "",
...
export async function provisionAndStart(
  ...
  if (isFreshProvision) {
    ...
    apiServerKey = await writeProfileEnv(profileName);
  } else {
