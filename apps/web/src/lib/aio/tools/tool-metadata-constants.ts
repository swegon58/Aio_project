import { ALL_GATEABLE_TOOLSETS, type PlanTier } from "@/lib/hermes/pricing";
...
export const AIO_TOOL_MANIFEST: AioToolManifestEntry[] = [
  ...
  },
  {
    ...
    version: AIO_TOOL_MANIFEST_VERSION,
    displayLabel: "Session Search",
    category: "toolset",
    owner: "hermes-gateway",
    inputSchema: "search query, or session id + anchor message id to scroll, or no args to browse recent sessions",
    outputSchema: "matched sessions with snippets/bookends, a message window, or a recent-sessions list",
    sideEffects: {
      reads: ["local"],
      writes: [],
      externalWrites: false,
    ...
    approvalPolicy: {
      ...
      sessionScopeAllowed: false,
      alwaysScopeAllowed: false,
      rationale: "Read-only FTS5 lookup over the user's own local session history; no LLM calls, no network, no writes.",
    },
    planAvailability: ALL_PLANS,
    notes: "R16-A4: not in ALL_GATEABLE_TOOLSETS — already registered in _HERMES_CORE_TOOLS and enabled by default for every tier, so no pricing.ts gating change is needed.",
  },
  {
    canonicalName: "web",
    version: AIO_TOOL_MANIFEST_VERSION,
  ...
  },
  {
    canonicalName: "moa",
    version: AIO_TOOL_MANIFEST_VERSION,
    displayLabel: "Deep Answer (Mixture-of-Agents)",
    category: "toolset",
    owner: "hermes-gateway",
    inputSchema: "user prompt, optional reference/aggregator model overrides",
    outputSchema: "synthesized answer text, models used, processing time",
    sideEffects: {
      reads: ["provider"],
      writes: [],
      externalWrites: false,
    ...
    approvalPolicy: {
      ...
      sessionScopeAllowed: false,
      alwaysScopeAllowed: false,
      rationale: "Sends the prompt to multiple external LLM providers for synthesis but does not mutate any external state.",
    },
    planAvailability: ["business"],
    notes: "R16-A3: 4 reference models + 1 aggregator = 5 OpenRouter calls per invocation, the highest per-call cost of any toolset — gated Business-only like image_gen/video_gen/tts. No dedicated 'deep answer' UI trigger exists; the LLM invokes this toolset autonomously once unlocked, same as delegation/vision/memory today.",
  },
  {
